    ...
    for (const auto &c : all_components) {
      ...
      if (fusion.count(key)) {
        fusion[key] = std::make_shared<tensorium::ASTNode>(
            tensorium::ASTNodeType::BinaryOp, "+",
            std::vector<tensorium::Node>{fusion[key], c.factor});
      } else {
        fusion[key] = c.factor;
      }
    }
...
void generate_metric_tensor_mlir(
  ...
  for (const auto &kv : fusion) {
    ...
    for (const auto &a : args)
      fout_symbolic << " " << a << ",";
    if (!args.empty())
      fout_symbolic.seekp(-1, std::ios_base::cur);
    ...
    for (size_t i = 0; i < args.size(); ++i)
      fout_symbolic << "f64, ";
    if (!args.empty())
      fout_symbolic.seekp(-2, std::ios_base::cur);
    fout_symbolic << " -> f64\n";
