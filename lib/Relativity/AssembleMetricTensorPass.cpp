...
namespace relativity {
	struct AssembleMetricTensorPass
		: public PassWrapper<AssembleMetricTensorPass, OperationPass<mlir::ModuleOp>> {
			...
			void runOnOperation() override {
				...
				for (auto func : module.getOps<mlir::func::FuncOp>()) { 
					for (auto &block : func.getBody()) {
						for (auto &op : block) {
							if (auto metric = llvm::dyn_cast<relativity::MetricComponentOp>(&op)) {
				...
				if (funcIt == module.getOps<mlir::func::FuncOp>().end()) return;
				...
				auto tensorOp = b.create<relativity::MetricTensorOp>(
						module.getLoc(), tensorTy, components
						);
			...
			StringRef getDescription() const final { return "Assembles full metric tensor from components"; }
