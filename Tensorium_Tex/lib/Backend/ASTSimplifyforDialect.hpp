...
inline Node simplify_expression(const Node &node) {
  ...
  if (node->type == ASTNodeType::UnaryOp) {
    ...
    if (node->value == "-" && arg->type == ASTNodeType::UnaryOp &&
        arg->value == "-")
      return arg->children[0];
    return std::make_shared<ASTNode>(ASTNodeType::UnaryOp, node->value,
                                     std::vector<Node>{arg});
  }
  ...
  if (node->type == ASTNodeType::BinaryOp && node->children.size() == 2) {
    ...
    if (lhs->type == ASTNodeType::Number && rhs->type == ASTNodeType::Number) {
      ...
      if (node->value == "+")
        return std::make_shared<ASTNode>(ASTNodeType::Number,
                                         std::to_string(a + b));
      if (node->value == "-")
        return std::make_shared<ASTNode>(ASTNodeType::Number,
                                         std::to_string(a - b));
    ...
    if (node->value == "+") {
      if (lhs->type == ASTNodeType::Number && lhs->value == "0")
        return rhs;
      if (rhs->type == ASTNodeType::Number && rhs->value == "0")
        return lhs;
    }
    if (node->value == "*") {
      if ((lhs->type == ASTNodeType::Number && lhs->value == "0") ||
      ...
      if (lhs->type == ASTNodeType::Number && lhs->value == "1")
        return rhs;
      if (rhs->type == ASTNodeType::Number && rhs->value == "1")
        return lhs;
    }
    if (node->value == "-") {
      if (rhs->type == ASTNodeType::Number && rhs->value == "0")
        return lhs;
      if (lhs->type == ASTNodeType::Number && lhs->value == "0")
        return std::make_shared<ASTNode>(ASTNodeType::UnaryOp, "-",
                                         std::vector<Node>{rhs});
    }

    return std::make_shared<ASTNode>(ASTNodeType::BinaryOp, node->value,
