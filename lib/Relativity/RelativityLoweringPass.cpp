...
Value resolveSymbol(const std::string &name, mlir::PatternRewriter &rewriter,
  ...
  if (name == "sin" || name == "cos" || name == "tan") {
    llvm::errs() << "[WARNING] Function " << name
                 << " called as symbol. Patching as 0.0\n";
...
mlir::Value emitFormula(tensorium::ASTNode *node,
  ...
  case NT::BinaryOp: {
    ...
    mlir::Value lhs =
        emitFormula(node->children[0].get(), rewriter, loc, inputs);
    mlir::Value rhs =
        emitFormula(node->children[1].get(), rewriter, loc, inputs);
    if (node->value == "+")
  ...
  case NT::UnaryOp: {
    ...
    if (node->value == "-")
      return rewriter.create<arith::NegFOp>(loc, s);
...
struct MetricTensorLowering
    : public OpRewritePattern<relativity::MetricTensorOp> {
  using OpRewritePattern::OpRewritePattern;
  ...
  LogicalResult matchAndRewrite(relativity::MetricTensorOp op,
    ...
    SmallVector<Value, 16> elems = {g00,  zero, zero, zero, zero, g11,
                                    zero, zero, zero, zero, g22,  zero,
                                    zero, zero, zero, g33};

    auto fromElem =
        rewriter.create<tensor::FromElementsOp>(loc, rankedTy, elems);
...
struct LowerRelativityPass
    : public PassWrapper<LowerRelativityPass, OperationPass<ModuleOp>> {
  void runOnOperation() override {
    ...
    if (!module.lookupSymbol<func::FuncOp>("metric_component_impl")) {
      ...
      FunctionType type = b.getFunctionType(
          SmallVector<Type, 7>(7, b.getF64Type()), b.getF64Type());
      b.create<func::FuncOp>(module.getLoc(), "metric_component_impl", type)
          .setPrivate();
    }
  ...
  StringRef getArgument() const override { return "lower-relativity"; }
